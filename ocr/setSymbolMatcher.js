...
// FIX: expanded y-jitter — the set symbol's vertical position shifts 5-15px
// depending on card type (creatures vs instants). ±2px was not enough.
// x position is stable so x-offsets are dropped entirely.
...
async function ensureSetSymbolCache(forceRefresh = false) {
  ...
  // FIX: exclude set types that never appear on playable card faces.
  // Token, memorabilia, and art_series sets pollute the hash pool and cause
  // false matches — e.g. Avatar token set "ttle" was hashing close to
  // almost everything and dominating every symbol match result.
  ...
  for (const set of sets) {
    ...
    const setType    = String(set?.set_type    || "").trim().toLowerCase();

    if (!setCode || !iconSvgUri) continue;
    if (EXCLUDED_SET_TYPES.has(setType)) continue;
